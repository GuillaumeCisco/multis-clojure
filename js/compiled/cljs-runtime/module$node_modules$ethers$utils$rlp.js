...
1+lengthLength,offset+1+lengthLength+length));return{consumed:1+lengthLength+length,result:data}}if(128<=data[offset]){length=data[offset]-128;if(offset+1+length>data.length)throw Error("invlaid rlp data");data=bytes_1.hexlify(data.slice(offset+1,offset+1+length));return{consumed:1+length,result:data}}return{consumed:1,result:bytes_1.hexlify(data[offset])}}Object.defineProperty(exports,"__esModule",{value:!0});var bytes_1=require("module$node_modules$ethers$utils$bytes");exports.encode=function(object){return bytes_1.hexlify(_encode(object))};
