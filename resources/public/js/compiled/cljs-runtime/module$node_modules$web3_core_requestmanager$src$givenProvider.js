...
global.web3.currentProvider.connection&&"ipcProviderWrapper"===global.web3.currentProvider.connection.constructor.name&&(global.web3.currentProvider.on=function(type,callback){if("function"!==typeof callback)throw Error("The second parameter callback must be a function.");switch(type){case "data":this.connection.on("data",function(data){var result="";data=data.toString();try{result=JSON.parse(data)}catch(e$4){return callback(Error("Couldn't parse response data"+data))}result.id||-1===result.method.indexOf("_subscription")||
